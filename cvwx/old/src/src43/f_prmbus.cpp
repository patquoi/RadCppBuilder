   ...
   for(i=1; i<=NBMAXLIGNESBUSTRAM; i++)
    {
     if (cv->NbArretsBus<NBMAXARRETSBUSTRAMPARLIGNE)
      ((TSPINEDIT*)FindComponent(asDebutNomSpinEdit+IntToStr(i)))->MAXVALUE=cv->NbArretsBus;
     ((TSPINEDIT*)FindComponent(asDebutNomSpinEdit+IntToStr(i)))->Value=cv->LigneBus[i-1].NbArrets;
     ...
     for(j=1; j<=NBMAXARRETSBUSTRAMPARLIGNE; j++)
      {
       ((TSPINEDIT*)FindComponent(asDebutNomSpinEdit+IntToStr(10*i+j)))->MAXVALUE=cv->NbArretsBus;
       ((TSPINEDIT*)FindComponent(asDebutNomSpinEdit+IntToStr(10*i+j)))->Value=cv->LigneBus[i-1].NumArret[j-1];
...
#ifdef CV_LINUX // v3.8. Permet de rafraîchir les SpinEdit des numéros de lignes (OnClick != OnChange)
 for(i=1; i<=NBMAXLIGNESBUSTRAM; i++)
...
void __fastcall TfrmParamBus::FormResize(TObject *Sender)
{
#ifdef CV_LINUX
 static bool RedimensionnementEnCours=false;
 int l=max( LMAX(BitBtn32,CSpinEdit59),
            LMAX(GroupBox3,LabelRqCochesLignesBus))+LH_ESPACE,
