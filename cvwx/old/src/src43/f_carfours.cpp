...
void __fastcall TfrmCarrefours::SpeedButtonClick(TObject *Sender)
{
 int i, x, y, x1, y1, x2, y2, x3, y3, d1, d2, d3,
     ...
     Type=mrRep%10,
     NumFeux[4];
 if ((Dimensions<44)&&(Type>1)&&(cv->NbFeuxLibres()<3))
  {
   AfficheMessage(Format(asNbFeuxLibresInsuffisant, ARRAYOFCONST((3))), asImpossibleCreerCarrefour, tmAvertissement, 0, 0);
   ModalResult=mrCancel;
   return;
  }
 if ((Dimensions==44)&&(Type>1)&&(cv->NbFeuxLibres()<4))
  {
   AfficheMessage(Format(asNbFeuxLibresInsuffisant, ARRAYOFCONST((4))), asImpossibleCreerCarrefour, tmAvertissement, 0, 0);
   ModalResult=mrCancel;
   return;
  }
...
//---------------------------------------------------------------------------
void __fastcall TfrmCarrefours::FormResize(TObject *Sender)
{
#ifdef CV_LINUX
 static bool RedimensionnementEnCours=false;
 int l=max(LMAX(GroupBox44,GroupBox66),
           XMAX(GroupBox88))+LH_ESPACE,
     h=HMAX(GroupBox43,GroupBox88)+LH_ESPACE;
...
//---------------------------------------------------------------------------
void __fastcall TfrmCarrefours::FormShow(TObject *Sender)
{
#ifdef CV_LINUX
 FormResize(this);
#endif
